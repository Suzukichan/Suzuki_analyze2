...
class AnalysisApp:
    def __init__(self, root):
        ...
        title_label = tk.Label(
            root, text="銘柄分析・スクリーニング", font=("Arial", 14, "bold")
        )
        ...
        # セクター分析ボタン
        self.btn_sector_analysis = tk.Button(
            sector_frame,
            text="セクター分析を実行",
            width=25,
            command=self.run_sector_analysis,
        )
        ...
        self.tree_sector = ttk.Treeview(
            sector_frame, columns=columns_sec, show="headings", height=8
        )
        ...
        for col in columns_sec:
            ...
            self.tree_sector.heading(
                col, command=lambda c=col: self.sort_tree(self.tree_sector, c, True)
            )
        ...
        self.sector_combo = ttk.Combobox(
            control_frame, values=sectors, state="readonly", width=25
        )
        self.sector_combo.pack(side=tk.LEFT, padx=5)
        if sectors:
            self.sector_combo.current(0)

        self.btn_sector_compare = tk.Button(
            control_frame, text="セクター内比較を実行", command=self.run_sector_compare
        )
        ...
        self.tree_compare = ttk.Treeview(
            compare_frame, columns=columns_comp, show="headings", height=8
        )
        ...
        for col in columns_comp:
            ...
            self.tree_compare.heading(
                col, command=lambda c=col: self.sort_tree(self.tree_compare, c, True)
            )
    ...
    def sort_tree(self, tree, col, descending):
        """Treeviewの列をクリックしたときにソートする処理"""
        targets = [(tree.set(k, col), k) for k in tree.get_children("")]

        # 数値列かどうかの判定用（%, N/A などを考慮）
        is_numeric = True
        ...
        def numeric_key(item):
            val = item[0]
            if val in ("N/A", "", "-"):
                # 降順のときは'N/A'などを一番下に、昇順のときも表示位置を考慮
                return float("-inf") if descending else float("inf")
            return float(val.replace("%", "").replace(",", ""))

        if is_numeric:
            targets.sort(key=numeric_key, reverse=descending)
        else:
            targets.sort(key=lambda x: x[0], reverse=descending)

        # Treeviewのアイテムを並べ替えた順序で再配置
        for index, (_, k) in enumerate(targets):
            tree.move(k, "", index)

        # 次回クリック時は昇順・降順を反転させる
        tree.heading(col, command=lambda: self.sort_tree(tree, col, not descending))
    ...
    def run_sector_analysis(self):
        ...
        if "市場全体" in sector_avg.index:
            ...
            for sector, row in sector_avg.iterrows():
                ...
                # 日経平均との比率(相対強度)
                rel_mom5 = (
                    row["Mom5"] / nikkei_row["Mom5"]
                    if nikkei_row["Mom5"] != 0
                    else np.nan
                )
                rel_mom10 = (
                    row["Mom10"] / nikkei_row["Mom10"]
                    if nikkei_row["Mom10"] != 0
                    else np.nan
                )
                rel_vol = (
                    row["VolRatio"] / nikkei_row["VolRatio"]
                    if nikkei_row["VolRatio"] != 0
                    else np.nan
                )

                # ③ 表で表示
                self.tree_sector.insert(
                    "",
                    "end",
                    values=(
                        sector,
    ...
    def run_sector_compare(self):
        ...
        if filtered.empty:
            messagebox.showinfo(
                "情報", f"「{target_sector}」のデータが見つかりません。"
            )
            return
